...
void loop() {
  ...
  if(millis() > nextPubMillis) {        // If it is next measurement time
    ...
    if (isnan(h) || isnan(t)) {
      Serial.println("Cant get data from sensor");
    } else {
      // Compute heat index in Celsius (isFahreheit = false)
      float hic = dht.computeHeatIndex(t, h, false);

      // If values was changed, upload them (and save for future reference)
      saveAndUploadReading(temperatureTopic, temperature, t);
      saveAndUploadReading(humidityTopic, humidity, h);
      saveAndUploadReading(heatIndexTopic, heatIndex, hic);
